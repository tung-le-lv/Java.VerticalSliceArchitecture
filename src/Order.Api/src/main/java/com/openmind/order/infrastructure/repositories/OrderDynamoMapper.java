...
final class OrderDynamoMapper {
    ...
    OrderAggregate toOrder(Map<String, AttributeValue> item) {
        try {
            List<OrderItemData> itemsData = objectMapper.readValue(
                    item.get("items").s(), new TypeReference<List<OrderItemData>>() {
                    });
            List<OrderItem> orderItems = itemsData.stream()
                    .map(i -> OrderItem.reconstitute(i.getProductId(), i.getProductName(), i.getQuantity(), i.getUnitPrice()))
                    .toList();
    ...
    Map<String, AttributeValue> toAttributeValues(OrderAggregate order) {
        try {
            List<OrderItemData> itemsData = order.getItems().stream()
                    .map(i -> new OrderItemData(i.productId(), i.productName(), i.quantity(), i.unitPrice().getAmount()))
                    .toList();

            Map<String, AttributeValue> item = new LinkedHashMap<>();
            item.put("id", AttributeValue.fromS(order.getId()));
            item.put("orderDate", AttributeValue.fromS(
                    LocalDate.ofInstant(order.getCreatedAt(), ZoneOffset.UTC).format(DATE_FORMAT)));
