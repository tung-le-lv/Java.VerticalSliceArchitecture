...
public class ProcessPaymentCommandHandler implements RequestHandler<ProcessPaymentCommand, ProcessPaymentResult>
{
    ...
    public ProcessPaymentCommandHandler(PaymentRepository paymentRepository, PaymentGateway paymentGateway,
            EventBus eventBus)
    {
    ...
    public ProcessPaymentResult handle(ProcessPaymentCommand request)
    {
        try
        {
            PaymentAggregate payment = PaymentAggregate.create(request.orderId(), request.customerId(),
                    request.amount());
