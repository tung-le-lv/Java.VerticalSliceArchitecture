...
import com.openmind.payment.shared.ValidationException;
 ...
 * matching a request's runtime type. This service has no validators in the
 * original (no FluentValidation dependency), so the validator map will simply
 * stay empty here.
 */
@Component
public class SimpleMediator implements Mediator
{
    ...
    public SimpleMediator(List<RequestHandler<?, ?>> handlerBeans, List<RequestValidator<?>> validatorBeans)
    {
        ...
        {
            Class<?> requestType = ResolvableType.forClass(RequestValidator.class, validator.getClass()).getGeneric(0)
                    .resolve();
            if (requestType == null)
            {
                throw new IllegalStateException("Cannot resolve request type for validator " + validator.getClass());
            }
            validators.computeIfAbsent(requestType, key -> new ArrayList<>()).add((RequestValidator<Object>) validator);
        }
    }
